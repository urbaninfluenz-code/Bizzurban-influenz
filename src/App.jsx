...
// Slider defaults per preset — what each preset "sounds like" by default
const PRESET_SLIDER_DEFAULTS = {
  cinematic: { reverb: 0.55, bass: 2,   brightness: 1,   warmth: 0.30, compression: 0.50, noiseReduction: 0.30 },
  rich:      { reverb: 0.22, bass: 3,   brightness: 0,   warmth: 0.50, compression: 0.75, noiseReduction: 0.35 },
  dolby:     { reverb: 0.60, bass: 1,   brightness: 2,   warmth: 0.15, compression: 0.35, noiseReduction: 0.25 },
  instagram: { reverb: 0.20, bass: 1,   brightness: 1.5, warmth: 0.25, compression: 0.70, noiseReduction: 0.40 },
  phone:     { reverb: 0.12, bass: 0,   brightness: 1,   warmth: 0.20, compression: 0.80, noiseReduction: 0.40 },
  auto:      { reverb: 0.35, bass: 1,   brightness: 0.5, warmth: 0.25, compression: 0.55, noiseReduction: 0.30 },
  custom:    { reverb: 0.35, bass: 1,   brightness: 0.5, warmth: 0.25, compression: 0.55, noiseReduction: 0.30 },
};

// Platform → suggested preset (hint only, not forced)
const PLATFORM_PRESET_HINT = {
  instagram: 'instagram',
  tiktok:    'instagram',
  youtube:   'cinematic',
  phone:     'phone',
...
function StatTile({ label, value, sub, accent = '#a78bfa' }) {
  return (
    <div className="glass-soft" style={{ padding: '14px 16px', minWidth: 0 }}>
      <p style={{ color: '#64748b', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', margin: '0 0 6px' }}>
        {label}
      </p>
      <p className="mono" style={{ color: '#f1f5f9', fontSize: '1.35rem', fontWeight: 700, margin: 0, lineHeight: 1.1 }}>
        {value}
      </p>
...
function VerticalSlider({ label, icon: Icon, value, min, max, step, onChange, displayValue, accent = '#a855f7', modified = false, disabled = false }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '10px 6px', minWidth: 0, opacity: disabled ? 0.45 : 1, transition: 'opacity 0.2s ease' }}>
      <div style={{ position: 'relative', width: 32, height: 32, borderRadius: 10, marginBottom: 8, background: `${accent}1f`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: accent }}>
        <Icon size={15} />
        {modified && (
          <div style={{ position: 'absolute', top: -3, right: -3, width: 8, height: 8, borderRadius: '50%', background: accent, boxShadow: `0 0 6px ${accent}`, border: '1.5px solid #05050f' }} title="Modified from preset default" />
        )}
      </div>
      ...
      <input
        ...
        aria-label={label}
      />
      <div style={{ marginTop: 6, width: '100%', height: 3, background: 'rgba(255,255,255,0.04)', borderRadius: 2, position: 'relative', overflow: 'hidden' }}>
...
function Toast({ message, color }) {
  return (
    <div className="animate-fade-in" style={{
      ...
      padding: '10px 20px', borderRadius: 999, zIndex: 9999,
      background: 'rgba(15,15,30,0.92)', backdropFilter: 'blur(14px)',
      border: `1px solid ${color}40`, color: '#f1f5f9',
...
export default function App() {
  ...
  const [noiseReduction, setNoiseReduction] = useState(PRESET_SLIDER_DEFAULTS.cinematic.noiseReduction);
  ...
  const presetFlashRef  = useRef(null); // tracks last flashed preset
  ...
  // Modified-from-default dots
  const defaults = PRESET_SLIDER_DEFAULTS[preset] || PRESET_SLIDER_DEFAULTS.auto;
  const modReverb = !approxEq(reverbAmount, defaults.reverb);
  const modBass   = !approxEq(bassBoost, defaults.bass);
  const modBright = !approxEq(brightness, defaults.brightness);
  const modWarmth = !approxEq(warmth, defaults.warmth);
  const modComp   = !approxEq(compression, defaults.compression);
  const anyModified = modReverb || modBass || modBright || modWarmth || modComp;

  // ── Toast helper ─────────────────────────────────────────────
  const showToast = useCallback((message, color = '#a855f7') => {
  ...
  // ── Auto-scroll to output when render finishes ────────────────
  useEffect(() => {
    if (outputBlob && outputCardRef.current) {
      setTimeout(() => outputCardRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 120);
    }
  }, [outputBlob]);
  ...
  // ── Waveform ─────────────────────────────────────────────────
  const drawWaveform = useCallback((buf, positionSec = -1) => {
    ...
    ctx.globalAlpha = 1;
    // playhead
    if (positionSec >= 0 && buf.duration > 0) {
      const x = (positionSec / buf.duration) * W;
      ctx.strokeStyle = 'rgba(255,255,255,0.65)';
      ctx.lineWidth = 1.5;
      ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, H); ctx.stroke();
      ctx.fillStyle = '#a855f7';
      ctx.beginPath(); ctx.arc(x, amp, 4, 0, Math.PI * 2); ctx.fill();
    }
  }, []);

  // Redraw waveform on position change during playback
  useEffect(() => {
    if (audioBuffer && isPlaying) drawWaveform(audioBuffer, position);
  }, [position, isPlaying, audioBuffer, drawWaveform]);

  // ── Engine teardown ──────────────────────────────────────────
  const teardownEngine = useCallback(() => {
    if (positionTimerRef.current) { clearInterval(positionTimerRef.current); positionTimerRef.current = null; }
  ...
  const applySliderDefaults = useCallback((id, silent = false) => {
    ...
    setReverbAmount(d.reverb);
    setBassBoost(d.bass);
    setBrightness(d.brightness);
    setWarmth(d.warmth);
    setCompression(d.compression);
    setNoiseReduction(d.noiseReduction);
    const eng = engineRef.current;
    if (eng) {
      eng.setReverb(d.reverb);
      eng.setBass(d.bass);
      eng.setBrightness(d.brightness);
      eng.setWarmth(d.warmth);
      eng.setCompression(d.compression);
    }
    if (!silent) {
      const meta = PRESET_META.find(p => p.id === id);
      const name = ENGINE_PRESETS[id === 'custom' ? 'auto' : id]?.name || 'Custom';
      showToast(`${name} defaults loaded`, meta?.color || '#a855f7');
    }
  }, [showToast]);
  ...
  const handleFile = useCallback(async (f) => {
    ...
    if (prevOutputUrlRef.current) { URL.revokeObjectURL(prevOutputUrlRef.current); prevOutputUrlRef.current = null; }
    setOutputUrl(null);
    teardownEngine();
    setFile(f);
    ...
    try {
      ...
      console.error(err);
      setError('Could not decode this file. Try mp3, wav, m4a, flac, or ogg.');
    }
  ...
  const clearFile = () => {
    teardownEngine();
    setFile(null); setFileInfo(null); setAudioBuffer(null); setAnalysis(null);
  ...
  const handlePlatformChange = useCallback((id) => {
    setPlatform(id);
    // Suggest matching preset if current one doesn't match the platform's optimal
    const suggested = PLATFORM_PRESET_HINT[id];
    if (suggested && suggested !== preset) {
      showToast(`Tip: Try "${ENGINE_PRESETS[suggested]?.name}" preset for ${PLATFORM_TARGETS[id].label}`, '#64748b');
    }
  }, [preset, showToast]);

  // ── Reset sliders ────────────────────────────────────────────
  const handleResetSliders = () => applySliderDefaults(preset, false);
  ...
  const handleSeek = (e) => {
    ...
    if (audioBuffer) drawWaveform(audioBuffer, newPos);
  };
  ...
  const handleRender = async () => {
    ...
    try {
      const config = { preset, customPrompt, platform, reverbAmount, bassBoost, brightness, warmth, compression, noiseReduction, autoTrim, trimRange: autoTrim ? (analysis?.suggestedTrim || null) : null };
      ...
      setOutputAnalysis({ lufs: blob.__lufs, peak: blob.__peak });
      setProgress(100); setProgressStep('Done');
      showToast('Master complete — ready to download', '#10b981');
    } catch (err) {
      console.error(err);
  ...
  const toggleOutputPlayback = () => {
    ...
    if (outputPlaying) { outputAudioRef.current.pause(); setOutputPlaying(false); }
    else { outputAudioRef.current.play(); setOutputPlaying(true); }
  };
  ...
  const trimSuggestion = useMemo(() => {
    ...
    const head = analysis.suggestedTrim.start;
    const tail = audioBuffer.duration - analysis.suggestedTrim.end;
    ...
    <div className="min-h-screen text-white" style={{ background: '#05050f', minHeight: '100vh' }}>
      ...
      <div className="relative" style={{ maxWidth: 880, margin: '0 auto', padding: '40px 20px 60px' }}>
        ...
        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 36 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              ...
              <p style={{ color: '#64748b', fontSize: '0.78rem', margin: '4px 0 0', fontWeight: 500 }}>Pro-grade audio mastering, in your browser</p>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            {audioBuffer && (
              <div className="animate-fade-in" style={{ padding: '5px 10px', borderRadius: 999, background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.25)', color: '#34d399', fontSize: '0.68rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <div
          ...
          style={{
            borderRadius: 20,
            border: `2px ${isDragging ? 'solid' : 'dashed'} ${isDragging ? '#a855f7' : file ? 'rgba(168,85,247,0.3)' : 'rgba(255,255,255,0.1)'}`,
            ...
            <div style={{ padding: 20 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, background: 'rgba(168,85,247,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {analysis && audioBuffer && (
          <div className="glass animate-fade-in" style={{ padding: 20, marginBottom: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <p style={{ color: '#94a3b8', fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <p style={{ color: '#94a3b8', fontSize: '0.72rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(138px, 1fr))', gap: 10 }}>
            {PRESET_META.map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => handlePresetChange(p.id)} className="lift" style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                    <div style={{ width: 32, height: 32, borderRadius: 9, background: active ? p.color + '28' : 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s ease' }}>
                      <Icon size={15} color={active ? p.color : '#64748b'} />
                    </div>
                    {(p.tag === 'Pro' || p.tag === 'AI' || p.tag === 'Social' || p.tag === 'Mobile') && (
                  ...
                  {active && (
                    <div style={{ position: 'absolute', bottom: 10, right: 10, width: 6, height: 6, borderRadius: '50%', background: p.color, boxShadow: `0 0 8px ${p.color}` }} />
                  )}
                </button>
              );
            })}
            {/* Custom prompt card */}
            <button onClick={() => handlePresetChange('custom')} className="lift" style={{
              padding: '14px 12px', borderRadius: 16,
              ...
              <div style={{ width: 32, height: 32, borderRadius: 9, marginBottom: 10, background: preset === 'custom' ? 'rgba(244,114,182,0.25)' : 'rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            <span className="mono" style={{ color: '#c4b5fd', fontSize: '0.78rem', fontWeight: 600, transition: 'all 0.2s ease' }}>
          ...
          <div className="segmented">
            {PLATFORM_META.map(p => {
              ...
              return (
                <button key={p.id} className={active ? 'active' : ''} onClick={() => handlePlatformChange(p.id)} title={`${PLATFORM_TARGETS[p.id].label} — ${PLATFORM_TARGETS[p.id].lufs} LUFS`}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}>
                    <Icon size={11} /> <span>{p.short}</span>
                  </span>
                </button>
              );
            })}
        ...
        <div className="glass" style={{ padding: 20, marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {anyModified && (
                <span className="animate-fade-in" style={{ fontSize: '0.66rem', fontWeight: 600, padding: '2px 8px', borderRadius: 999, background: 'rgba(251,146,60,0.12)', color: '#fb923c', border: '1px solid rgba(251,146,60,0.2)' }}>
                  Modified
                </span>
              )}
              <span style={{ fontSize: '0.68rem', fontWeight: 600, padding: '3px 10px', borderRadius: 999, background: (currentPresetMeta?.color || '#a855f7') + '18', color: currentPresetMeta?.color || '#c4b5fd', border: `1px solid ${(currentPresetMeta?.color || '#a855f7')}30` }}>
                {currentPresetCfg?.name || 'Custom'}
              </span>
              <button onClick={handleResetSliders} title="Reset sliders to preset defaults (loads preset sound)" style={{ width: 26, height: 26, borderRadius: 8, border: 'none', background: 'rgba(255,255,255,0.04)', color: '#64748b', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s ease' }}
                onMouseOver={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.1)'; e.currentTarget.style.color = '#c4b5fd'; }}
          ...
          {/* "Upload first" hint when no file */}
          {!audioBuffer && (
            <div style={{ textAlign: 'center', padding: '14px 0 6px', color: '#334155', fontSize: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              <Upload size={14} color="#334155" /> Upload audio to enable live preview
            </div>
          )}

          {/* Transport */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
            <button onClick={togglePlayback} disabled={!audioBuffer} className={isPlaying ? 'animate-pulse-ring' : ''} style={{
              width: 50, height: 50, borderRadius: '50%', flexShrink: 0,
              background: audioBuffer ? 'linear-gradient(135deg, #a855f7, #06b6d4)' : 'rgba(255,255,255,0.05)',
            ...
            }} title={audioBuffer ? 'Play / Pause (Space)' : 'Upload audio first'}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
            <div style={{ flex: 1 }}>
              <div className="meter-track" style={{ overflow: 'visible' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                {['-60','-30','-12','-3','0 dB'].map(l => (
                  <span key={l} className="mono" style={{ color: '#475569', fontSize: '0.66rem' }}>{l}</span>
                ))}
              </div>
            </div>
            <div style={{ textAlign: 'right', minWidth: 90 }}>
              <p className="mono" style={{ color: isClipping ? '#ef4444' : '#f1f5f9', fontSize: '1.3rem', fontWeight: 700, margin: 0, lineHeight: 1, transition: 'color 0.1s ease', textShadow: isClipping ? '0 0 12px rgba(239,68,68,0.6)' : 'none' }}>
                {fmtDb(meter.peakDb)} dB
              </p>
              <p style={{ color: isClipping ? '#ef4444' : '#64748b', fontSize: '0.66rem', margin: '2px 0 0', fontWeight: 600, transition: 'color 0.1s ease' }}>
                {isClipping ? 'Clipping!' : 'Live peak'}
              </p>
            </div>
          </div>
          ...
          <div className="slider-grid" style={{ padding: '12px 6px', background: 'rgba(0,0,0,0.2)', borderRadius: 14, marginBottom: 12 }}>
            <VerticalSlider label="Reverb"      icon={Waves}     value={reverbAmount} min={0}  max={1}  step={0.01} onChange={setReverbAmount} displayValue={`${Math.round(reverbAmount * 100)}%`}      accent="#a855f7" modified={modReverb} disabled={!audioBuffer} />
            <VerticalSlider label="Bass"        icon={Volume2}   value={bassBoost}    min={-6} max={10} step={0.5}  onChange={setBassBoost}    displayValue={`${fmtDb(bassBoost, 1)} dB`}              accent="#ec4899" modified={modBass}   disabled={!audioBuffer} />
            <VerticalSlider label="Brightness"  icon={Sparkles}  value={brightness}   min={-6} max={8}  step={0.5}  onChange={setBrightness}   displayValue={`${fmtDb(brightness, 1)} dB`}            accent="#22d3ee" modified={modBright} disabled={!audioBuffer} />
            <VerticalSlider label="Warmth"      icon={Film}      value={warmth}       min={0}  max={1}  step={0.01} onChange={setWarmth}       displayValue={`${Math.round(warmth * 100)}%`}           accent="#f59e0b" modified={modWarmth} disabled={!audioBuffer} />
            <VerticalSlider label="Punch"       icon={Zap}       value={compression}  min={0}  max={1}  step={0.01} onChange={setCompression}  displayValue={`${Math.round(compression * 100)}%`}      accent="#10b981" modified={modComp}   disabled={!audioBuffer} />
          </div>

          {/* Noise reduction */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.05)', opacity: audioBuffer ? 1 : 0.45, transition: 'opacity 0.2s ease' }}>
            <div style={{ width: 32, height: 32, borderRadius: 9, flexShrink: 0, background: 'rgba(34,211,238,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {showCustom && (
          <div className="glass animate-fade-in" style={{ padding: 18, marginBottom: 24 }}>
            ...
            <textarea value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} placeholder='e.g. "Deep cinematic reverb, warm vintage tape, punchy podcast voice"' rows={3} style={{ width: '100%', padding: '12px 14px', borderRadius: 12, background: 'rgba(0,0,0,0.25)', border: '1.5px solid rgba(244,114,182,0.25)', color: '#e2e8f0', fontSize: '0.85rem', outline: 'none', resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box' }} />
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
              {QUICK_TAGS.map(tag => (
                <button key={tag} onClick={() => setCustomPrompt(p => p ? `${p}, ${tag}` : tag)} style={{ fontSize: '0.72rem', padding: '4px 10px', borderRadius: 999, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', color: '#94a3b8', cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s ease' }} onMouseOver={e => { e.currentTarget.style.background = 'rgba(244,114,182,0.08)'; e.currentTarget.style.color = '#f9a8d4'; }} onMouseOut={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.03)'; e.currentTarget.style.color = '#94a3b8'; }}>
                  + {tag}
                </button>
              ))}
            </div>
        ...
        {/* Error */}
        {error && (
          <div className="animate-fade-in" style={{ marginBottom: 20, padding: '12px 16px', borderRadius: 12, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', color: '#fca5a5', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ flex: 1 }}>{error}</span>
            <button onClick={() => setError(null)} style={{ background: 'none', border: 'none', color: '#f87171', cursor: 'pointer', padding: 2 }}><X size={14} /></button>
          </div>
        )}
        ...
        <div style={{ marginBottom: 24 }}>
          {processing ? (
            ...
            <button onClick={handleRender} disabled={!audioBuffer} className={audioBuffer ? 'btn-process' : ''} style={{
              width: '100%', padding: '16px 20px', borderRadius: 16, border: 'none',
              cursor: audioBuffer ? 'pointer' : 'not-allowed',
              background: audioBuffer ? 'linear-gradient(135deg, #a855f7 0%, #ec4899 50%, #06b6d4 100%)' : 'rgba(255,255,255,0.05)',
              color: audioBuffer ? '#fff' : '#334155', fontFamily: 'inherit', fontWeight: 700, fontSize: '1rem',
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <Sparkles size={18} />
                {audioBuffer ? 'Master & Download' : 'Upload audio first'}
                {audioBuffer && <ChevronRight size={18} />}
              </span>
              {audioBuffer && (
                <span style={{ fontSize: '0.72rem', fontWeight: 500, opacity: 0.85 }}>
                  {currentPresetCfg?.name} · {platformTarget.label} · {platformTarget.lufs} LUFS
                </span>
              )}
            </button>
          )}
        ...
        {outputBlob && outputAnalysis && !processing && (
          <div ref={outputCardRef} className="glass animate-fade-in" style={{ padding: 22, marginBottom: 24, border: '1.5px solid rgba(16,185,129,0.25)', background: 'rgba(16,185,129,0.04)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
              <div style={{ width: 40, height: 40, borderRadius: 12, background: 'rgba(16,185,129,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <CheckCircle2 size={20} color="#34d399" />
              </div>
              <div>
                <p style={{ color: '#34d399', fontWeight: 700, fontSize: '0.95rem', margin: 0 }}>Master Complete</p>
                <p style={{ color: '#64748b', fontSize: '0.78rem', margin: '2px 0 0' }}>Optimized for {platformTarget.label}</p>
              </div>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 16 }}>
              ...
              <StatTile label="Status" value="Ready" sub="Click download" accent="#34d399" />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderRadius: 12, background: 'rgba(0,0,0,0.25)', marginBottom: 14 }}>
              <button onClick={toggleOutputPlayback} style={{ width: 38, height: 38, borderRadius: '50%', flexShrink: 0, background: 'rgba(16,185,129,0.18)', border: '1px solid rgba(16,185,129,0.3)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ color: '#64748b', fontSize: '0.72rem', margin: 0 }}>WAV · 16-bit · LUFS-normalized</p>
              </div>
              <audio ref={outputAudioRef} src={outputUrl} onEnded={() => setOutputPlaying(false)} style={{ display: 'none' }} />
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={handleDownload} style={{ flex: 1, padding: '12px 16px', borderRadius: 12, background: 'linear-gradient(135deg, #10b981, #06b6d4)', border: 'none', cursor: 'pointer', color: '#fff', fontWeight: 700, fontSize: '0.9rem', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 0 22px rgba(16,185,129,0.3)' }}>
                <Download size={16} /> Download .wav
              </button>
              <button onClick={resetForAnother} style={{ padding: '12px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', color: '#cbd5e1', fontWeight: 600, fontSize: '0.85rem', fontFamily: 'inherit', display: 'flex', alignItems: 'center', gap: 6 }}>
